...
def static_analysis_with_materials(
    ...
    output_dir = os.path.dirname(output_displacement_file)
    if output_dir:
        os.makedirs(output_dir, exist_ok=True)
    
    if print_results:
        print("Starting static structural analysis...")
    ...
    if print_results:
        print(f"Material properties: E = {E:.2e} Pa, nu = {nu}")
    ...
    if print_results:
        print(f"Solving elasticity problem...")
    ...
    # ===== EXPORT RESULTS =====
    with XDMFFile(output_displacement_file) as xdmf:
        xdmf.parameters["flush_output"] = True
        xdmf.parameters["functions_share_mesh"] = True
        xdmf.write(u_sol)

    with XDMFFile(output_vonmises_file) as xdmf:
        xdmf.parameters["flush_output"] = True
        xdmf.parameters["functions_share_mesh"] = True
        xdmf.write(von_mises_sol)

    # ===== RESULTS SUMMARY =====
    if print_results:
        print(f"Analysis complete: Max displacement = {max_displacement:.6e} m, Max stress = {max_stress/1e6:.2f} MPa")
